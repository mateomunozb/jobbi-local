...
foto() {
  ...
  local G='jobbi_http_request_duration_seconds'
  {
    ...
    echo "   outbox pendientes              $(prom 'sum(max by (productor) (jobbi_outbox_pendientes))')"
    ...
    echo "   conexiones PostgreSQL          $(prom 'sum(max by (estado) (jobbi_postgres_conexiones))') de $(prom 'max(jobbi_postgres_conexiones_max)')"
